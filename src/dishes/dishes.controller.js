...
//middleware to check if the dish has a name. Needed in order to move onto 'create()' and 'update()'.

function bodyHasName(req, res, next) {
  // deconstruct name from request body.
  const { data: { name } = {} } = req.body;
  // if dish has a name, move request onto next function. If not, cease execution and return error status and message.
  if (name) {
...
//middleware to check if dish has description. Needed in order to move onto 'create()' and 'update()'.
function bodyHasDescription(req, res, next) {
  // deconstruct description from request body.
  const { data: { description } = {} } = req.body;
  // if description, move request to next function. If not, cease execution and return error status and message.
  if (description) {
...
//middleware to check if the dish has a price. needed in order to move onto 'create()' and 'update()'.
function bodyHasPrice(req, res, next) {
  //decronstruct price from request body.
  const { data: { price } = {} } = req.body;
  // if no price, move request to next function. If no price, return error status code with message
  if (price) {
...
//middleware to check if dish price provided is valid. //needed to move onto 'update() 'create()'
function bodyHasValidPrice(req, res, next) {
  //decronstruct price from request body.
  const { data: { price } = {} } = req.body;
  //if price is valid (price >= 0), move request to next function. If invalid price, return error status code with message.
  if (price > -1) {
    res.locals.price = price;
    return next();
  } else {
...
//middleware to check if dish price is valid for 'udpate()'. Needed to move onto 'update()' and 'create()'
function bodyHasValidPriceForUpdate(req, res, next) {
  //decronstruct price from request body.
  const { data: { price } = {} } = req.body;
  //if price is invalid (price <= 0 OR not an integer), return error status code with message.
...
//middleware to check if dish has an image property. Needed to move onto 'create()' and 'update()'.
function bodyHasImg(req, res, next) {
  //deconstruct image_url from request body.
  const { data: { image_url } = {} } = req.body;
  //if there is an image url, move on to next function.  If no image url, return object for error status code and message.
  if (image_url) {
